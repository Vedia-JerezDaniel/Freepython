...
overw = (df['weight'] / ((df['height'] / 100)**2) > 25).astype(int)
# print(overweight)
df['overw'] = overw
df.head()
...
def cat_plot():
    ...
    df_cat = pd.melt(
        df, id_vars=['cardio'],
        value_vars=['active', 'alco', 'cholesterol', 'gluc', 'overw', 'smoke'])
...
def heat_map():
    ...
    ax = sns.heatmap(corr, linewidths=.5, annot=True, fmt='.1f', mask=mask, square=True, center=0,
        vmin=-0.1, vmax=0.25,cbar_kws={'shrink': .45,'format': '%.2f'})
